...
exports.generateCode = async (req, res) => {
    ...
    try {
        const code = await callAI('deepseek/deepseek-coder-v2', [
...
exports.getMathReasoning = async (req, res) => {
    ...
    try {
        const reasoning = await callAI('deepseek/deepseek-chat', [
            { role: 'system', content: systemPrompt },
            { role: 'user', content: prompt }
        ]);
        res.json({ reasoning });
...
exports.solveCodingTask = async (req, res) => {
    ...
    try {
        const solution = await callAI('deepseek/deepseek-coder-v2', [
            { role: 'system', content: systemPrompt },
            { role: 'user', content: prompt }
        ]);
        res.json({ solution });
...
exports.handleChat = async (req, res) => {
    ...
    try {
        const response = await callAI('meta-llama/llama-3-8b-instruct', [
            { role: 'system', content: systemPrompt },
            { role: 'user', content: prompt }
        ]);
        res.json({ response });
...
exports.imageToText = async (req, res) => {
    ...
    try {
        ...
        const response = await openrouter.chat.completions.create({
            model: 'meta-llama/llama-3.1-8b-instruct', // Vision model
            messages: [{
                role: 'user',
                content: [{
                    type: 'image_url',
...
exports.getExplainer = async (req, res) => {
    ...
    try {
        const explanation = await callAI('deepseek/deepseek-chat', [
            { role: 'system', content: systemPrompt },
